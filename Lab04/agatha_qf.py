...
solver.add_assertion(ForAll([i,j], Iff((Function(richer, [i,j])),(Not(Function(richer, [j,i]))))))

# Charles hates no one that Agatha hates. 
solver.add_assertion(ForAll(i, Implies(Function(hates, [agatha,Int(i)]), Not(Function(hates, [charles,i])))))
...
solver.add_assertion(ForAll(i, Implies(Not(Function(richer, [Int(i),agatha])), Function(hates, [butler,Int(i)]))))

# The butler hates everyone whom Agatha hates. 
solver.add_assertion(ForAll(i, Implies(Function(hates, [agatha,Int(i)]), Function(hates, [butler,Int(i)]) )))

# Noone hates everyone. 
solver.add_assertion(ForAll(i, Plus([Ite(Function(hates, [Int(i),Int(j)]),Int(1),Int(0)) for j in range(n)]) <= Int(2)))
